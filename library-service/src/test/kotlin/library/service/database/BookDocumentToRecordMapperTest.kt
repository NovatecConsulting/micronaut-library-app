...
internal class BookDocumentToRecordMapperTest {
    ...
    @Test
    fun `mapping converts representations of the document's data to value types`() {
    ...
    @Nested
    inner class `handling of book states` {

        @Test
        fun `mapping sets 'available' state if borrowed value is null`() {
            with(cut.map(availableBookDocument)) {
        ...
        @Test
        fun `mapping sets 'borrowed' state if borrowed value is set`() {
